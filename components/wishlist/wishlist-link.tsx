...
export function WishlistLink() {
  ...
  return (
    // 44×44px tikvlak; -mx-2 compenseert in de layout (icoon blijft 20px).
    <Link
      href="/favorieten"
    ...
    >
      ...
      {wl.hydrated && wl.count > 0 ? (
        <span className="absolute right-1 top-1.5 inline-flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-ink px-1 text-[0.6rem] text-canvas">
          {wl.count}
        </span>
      ) : null}
    </Link>
