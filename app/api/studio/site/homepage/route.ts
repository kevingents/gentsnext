...
import { getHomeLayout, schoonHomeLayout, DEFAULT_HOME } from "@/lib/homepage";
...
export async function POST(req: Request) {
  ...
  let body: { secties?: unknown; version?: unknown };
  try {
    body = (await req.json()) as { secties?: unknown; version?: unknown };
  } catch {
  ...
  try {
    ...
    if (typeof body.version === "string" && body.version) {
      const huidig = docVersion((await getHomeLayout()).secties);
    ...
    const { secties } = await getHomeLayout();
    return NextResponse.json({ ok: true, secties, version: docVersion(secties) });
  } catch (e) {
    return NextResponse.json({ ok: false, error: (e as Error).message }, { status: 500 });
  }
}
