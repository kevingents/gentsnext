...
export function ProfileCompletionForm({ token, email }: { token: string; email: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !d.ok) throw new Error(d.error || "Er ging iets mis.");
  ...
  if (done) {
    return (
      <div className="mt-8 rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center">
        <p className="text-lg font-semibold text-emerald-800">{done.already ? "Je profiel is bijgewerkt" : "Gelukt — 50 punten erbij!"}</p>
        <p className="mt-1 text-sm text-emerald-700">{done.already ? "De bonus was al toegekend, maar je gegevens zijn nu compleet." : "De punten staan op je voucherkaart. Bedankt!"}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="mt-8 space-y-4">
      {email && <p className="text-sm text-neutral-500">Voor {email}</p>}
      <div className="grid grid-cols-2 gap-3">
        <input className={input} placeholder="Voornaam" value={f.firstName} onChange={set("firstName")} required />
        <input className={input} placeholder="Achternaam" value={f.lastName} onChange={set("lastName")} />
      </div>
      <input className={input} placeholder="Telefoon (optioneel)" value={f.phone} onChange={set("phone")} />
      <p className="pt-2 text-sm font-medium text-neutral-800">Je maten (optioneel — voor passend advies)</p>
      <div className="grid grid-cols-2 gap-3">
        <input className={input} placeholder="Colbert (bv. 50)" value={f.colbert} onChange={set("colbert")} />
        <input className={input} placeholder="Broek (bv. 33)" value={f.broek} onChange={set("broek")} />
        <input className={input} placeholder="Overhemd (bv. 41)" value={f.overhemd} onChange={set("overhemd")} />
        <input className={input} placeholder="Schoen (bv. 43)" value={f.schoen} onChange={set("schoen")} />
      </div>
      ...
      <button type="submit" disabled={busy} className="w-full rounded-lg bg-neutral-900 py-3 text-sm font-semibold text-white disabled:opacity-50">
        {busy ? "Bezig…" : "Profiel afronden — 50 punten"}
      </button>
    </form>
  );
}
