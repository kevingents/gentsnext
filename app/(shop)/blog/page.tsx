...
export default async function BlogIndex() {
  ...
  return (
    <div className="mx-auto max-w-page px-gutter py-12">
      <p className="label-brand">Stijlgids</p>
      <h1 className="mt-2 text-display-md">Advies &amp; inspiratie</h1>
      <p className="mt-3 max-w-prose font-sans text-ink-soft">
        Praktische stylingtips en dresscode-advies van onze stylisten — met de bijpassende stukken uit de collectie.
      </p>

      {posts.length === 0 ? (
        <p className="mt-10 font-sans text-ink-soft">Binnenkort verschijnen hier de eerste stijladviezen.</p>
      ) : (
        <div className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="group block">
              ...
              <p className="mt-3 label-brand">{p.occasion || "Stijlgids"}</p>
