...
/* ── steekproef en looptijd ────────────────────────────────────────────── */

/**
 * Hoeveel bezoekers heb je per variant nodig?
 *
 * Standaard tweezijdige toets op twee proporties, α=0,05 en power=0,80:
 *   n = (z(α/2) + z(β))² · (p1(1−p1) + p2(1−p2)) / (p2 − p1)²
 *
 * Dit getal is de enige eerlijke manier om vóóraf te weten of een test kán
 * slagen. Bij onze aantallen is het meestal het antwoord op "waarom staat er
 * na twee weken nog steeds geen uitslag": een aankooptest die 5% wil meten op
 * 2% basisconversie heeft tienduizenden bezoekers per kant nodig, en die
 * hebben we niet. Dan kies je een doel dat dichterbij ligt (winkelwagen,
 * doorklik) of een groter effect.
 */
export function benodigdeSteekproef(basisPct, relatieveVerbeteringPct) {
  const p1 = Number(basisPct) / 100;
  const lift = Number(relatieveVerbeteringPct) / 100;
  if (!(p1 > 0) || !(p1 < 1) || !(lift > 0)) return null;
  const p2 = Math.min(0.999999, p1 * (1 + lift));
  const verschil = p2 - p1;
  if (!(verschil > 0)) return null;
  const zAlpha = 1.959964; // tweezijdig, α = 0,05
  const zBeta = 0.841621; //  power = 0,80
  const n = ((zAlpha + zBeta) ** 2 * (p1 * (1 - p1) + p2 * (1 - p2))) / verschil ** 2;
  return Math.ceil(n);
}

/**
 * Mag er al een oordeel geveld worden?
 *
 * Dit is de rem op "peeking": wie elke dag naar een p-waarde kijkt en stopt
 * zodra die onder 0,05 duikt, vindt vroeg of laat altijd een winnaar — ook als
 * er niets aan de hand is. De rem is niet een strengere drempel maar het
 * simpelste dat écht werkt: geen uitslag tónen vóór de vooraf afgesproken
 * looptijd en steekproef gehaald zijn.
 *
 * Een volle week is het minimum omdat verkeer op zondag anders is dan op
 * dinsdag; korter meet je vooral welke dagen er toevallig in zaten.
 */
export function oordeelKlaar({ dagenGelopen, minDagen, kleinsteVariant, doelPerVariant }) {
  const dagen = Number(dagenGelopen) || 0;
  const nodigDagen = Number(minDagen) || 0;
  const kleinste = Number(kleinsteVariant) || 0;
  const nodigN = Math.max(100, Number(doelPerVariant) || 0);
  if (dagen < nodigDagen) {
    const rest = Math.ceil(nodigDagen - dagen);
    return { klaar: false, reden: `loopt nog ${rest} ${rest === 1 ? "dag" : "dagen"} — een volle week vangt het weekritme` };
  }
  if (kleinste < nodigN) {
    return { klaar: false, reden: `nog ${nodigN - kleinste} bezoekers nodig op de kleinste variant (${kleinste} van ${nodigN})` };
  }
  return { klaar: true, reden: "" };
}
...
function schoonOverrides(raw) {
  ...
  if (r.pdp && typeof r.pdp === "object") {
    ...
    zet("reviewsPositie", p.reviewsPositie === "boven" || p.reviewsPositie === "onder" ? p.reviewsPositie : "");
    zet("kortingLabel", aanUit(p.kortingLabel));
    zet("video", aanUit(p.video));
    zet("betaaliconen", aanUit(p.betaaliconen));
    // Aantal beelden in de galerij. Het onderzoek is eenduidig dat één sterk
    // beeld het wint van een lange carrousel, maar dat is per winkel anders —
    // dus meetbaar maken in plaats van aannemen. Nooit 0: dan is er geen PDP.
    const maxBeelden = Math.round(Number(p.galerijMax));
    if (Number.isFinite(maxBeelden) && maxBeelden >= 1 && maxBeelden <= 12) pdp.galerijMax = maxBeelden;
  ...
  if (r.plp && typeof r.plp === "object") {
    ...
    if (p.filterPositie === "zijkant" || p.filterPositie === "boven") plp.filterPositie = p.filterPositie;
    const snel = aanUit(p.snelToevoegen);
    if (snel) plp.snelToevoegen = snel;
...
function schoonExperiment(raw) {
  ...
  // Vooraf vastgelegde looptijd en steekproef — samen de rem op te vroeg
  // stoppen. Nul betekent "geen eigen eis"; de resultatenroute houdt sowieso
  // een ondergrens van 100 bezoekers per kant aan.
  const dagen = Math.round(Number(r.minLooptijdDagen));
  const minLooptijdDagen = Number.isFinite(dagen) ? Math.min(90, Math.max(0, dagen)) : 7;
  const steekproef = Math.round(Number(r.doelBezoekersPerVariant));
  const doelBezoekersPerVariant = Number.isFinite(steekproef) ? Math.min(1000000, Math.max(0, steekproef)) : 0;
  // Welk effect wil je kunnen zien? Relatief, dus 15 = "15% meer conversie".
  // Hieruit rekent de resultatenroute de benodigde steekproef; het dwingt de
  // vraag "is dit überhaupt meetbaar bij ons verkeer?" naar vóóraf.
  const effect = Number(r.verwachtEffectPct);
  const verwachtEffectPct = Number.isFinite(effect) && effect > 0 ? Math.min(500, Math.max(1, Math.round(effect))) : 15;
