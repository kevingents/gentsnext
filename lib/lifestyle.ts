...
/**
 * Sfeerbeeld-generatie per product voor de lerende sfeerbeeld-studio: één slot
 * opnieuw genereren vanuit een API-route. PUUR PREVIEW (lifestyle_image_url*) —
 * het live-beeld blijft staan tot iemand de kandidaat goedkeurt.
 *
 * Thema, camerastijl en merkregels komen uit lib/media-themes, hetzelfde als het
 * script en de cron. Dit bestand had een eigen KOPIE van MOODS/CAT/EVERYMAN; die
 * liep uit de pas zodra iemand een thema in de portal aanpaste — dan hergenereerde
 * de studio met het oude decor. Daarbovenop komen de geleerde stijlregels uit de
 * learnings-store (afkeuren + reden).
 */
...
export async function regenerateLifestyleSlot(handle: string, slot: 1 | 2 | 3): Promise<{ ok: true; url: string } | { ok: false; error: string }> {
  ...
  const basis = buildPrompt({ hoofdgroep: r.hg, color: r.vcl ?? "", title: r.title }, theme, camera);
  if (!basis) return { ok: false, error: `Geen merkregel voor hoofdgroep "${r.hg || "?"}".` };

  const learnings = await getVisualLearnings();
  const prompt = `${basis}${learningsPromptBlock(learnings)}`;
