...
/**
 * Anthropic (Claude) mag in de sandbox WEL naar buiten — een bewuste
 * uitzondering op de default-deny, met een andere reden dan blob/Neon.
 *
 * De maatstaf van dit slot is: kan een aanroep een klant bereiken, geld
 * verplaatsen of iets wijzigen bij een derde? AI-aanroepen kunnen dat geen van
 * drieën — het is een dienst die wij afnemen, geen kanaal naar buiten. Het
 * enige risico is de rekening, en dat hoort bij een budget (zie
 * lib/ai-rapportage.js), niet bij een blokkade. Zonder deze uitzondering zijn
 * de HQ-bot en de AI-rapportages in de sandbox dood, en juist dáár horen
 * collega's vrij te kunnen oefenen: de data is geanonimiseerd, dus elke vraag
 * is er AVG-vrij.
 *
 * OpenAI blijft dicht: er is geen sandbox-functie die het nodig heeft, en wat
 * niet nodig is blijft dicht — dat is de kern van default-deny.
 */
const AI_DOORLATEN = [/(^|\.)anthropic\.com$/];
...
export function installeerNetSlot() {
  ...
  globalThis.fetch = async function sandboxFetch(input, init) {
    ...
    if (past(host, AI_DOORLATEN)) return ECHTE_FETCH(verzoek.bron);
